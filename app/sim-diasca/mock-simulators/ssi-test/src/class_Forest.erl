...
%
% - the significant features of a sim-diasca actor scheduled totally in periodic
% mode. The periodic mode scheduling means all activities are spontaneously
% scheduled
%
...
%
-spec construct( wooper_state(), class_Actor:actor_settings(), string(),
...
construct( State, ?wooper_construct_parameters ) ->
	...
	StartingState = setAttributes( ActorState, [
		...
		{periodic,1},
		{cycle,12},
		{famine_alert_saisons,[3]},
		{savage_alert_saisons,[9]},
		{squirrel_reproduce_saisons,[5]},
		{fire_alert_saisons,[11]},
...
% (actor oneway)
%
-spec addInPeers( wooper_state(), class_name(), pid() ) ->
						class_Actor:actor_oneway_return().
...
% (actor oneway)
%
-spec requiredLocation( wooper_state(), pid() ) ->
							  class_Actor:actor_oneway_return().
...
% (actor oneway)
%
-spec beginCompetition( wooper_state(), pid() ) ->
							  class_Actor:actor_oneway_return().
...
% (actor oneway)
%
-spec informedParticipation( wooper_state(), pid(), number(), pid() ) ->
					class_Actor:actor_oneway_return().
...
% (const request)
%
-spec getVirtualProbe( wooper_state() ) -> request_return( pid() ).
...
% (const request)
%
-spec getDataLoggerPid( wooper_state() ) -> request_return( pid() ).
...
perform_spontaneous_action( State ) ->
	...
	FamineTriggers = ?getAttr(famine_alert_saisons),
	SavageTriggers = ?getAttr(savage_alert_saisons),
	FireTriggers   = ?getAttr(fire_alert_saisons),

	ReproductionTriggers = ?getAttr(squirrel_reproduce_saisons),
...
% (helper)
%
-spec notify_be_registered( wooper_state(), [pid()] ) -> wooper_state().
