...
-spec construct( wooper:state(), class_Actor:actor_settings(),
				 ...
				 waste_capacity(), burner_capacity(), pid() ) -> wooper:state().
construct( State, ActorSettings, Name, Location, CapacityInformation,
	...
	WasteStockProbePid = class_Actor:declare_probe(
				...
				_Title=text_utils:format( "Waste Storage & "
					"Treatment Monitoring for Incinerator ~s", [ Name ] ),
...
generate_definitions( IncineratorCount, LocationGeneratorPid, GISInfo ) ->
	...
	define_incinerators( IncineratorCount, GISInfo, _Acc=[] ).
...
generate_tanks( TankCount, Acc ) ->
	...
	Mass = erlang:abs(
			 class_RandomManager:get_gaussian_value( MeanMass, VarianceMass ) ),
...
manage_capacity_information( _CapacityInformation=[
	...
	TankDesc = text_utils:format( "Quantity of waste stored in "
...
% (helper)
%
-spec send_data_to_probe( wooper:state() ) -> basic_utils:void().
...
burners_to_string( Burners ) ->
	...
	text_utils:format( "~B burners:~s", [ length( BurnerList ),
...
to_string( State ) ->
	...
	text_utils:format( "Incinerator '~s' (AAI: ~B) located at ~s (~s)"
					   "making use of a ~s and relying on ~s, "
