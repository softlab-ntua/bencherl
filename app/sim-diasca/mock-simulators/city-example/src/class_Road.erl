...
-spec getTargetPOI( wooper:state() ) ->
						 request_return( { class_Actor:aai(), pid() } ).
getTargetPOI( State ) ->

	% To help the caller multiplex such requests yet be still reproducible:
	MyAAI = class_Actor:get_abstract_identifier( State ),

	?wooper_return_state_result( State, { MyAAI, ?getAttr(target_poi) } ).
...
to_string( State ) ->
	...
	text_utils:format( "Road '~s' (AAI: ~B) from POI ~p (~w) to POI ~p (~w), "
					   "whose length is ~f meters,"
					   " whose current capacity is ~B/~B, whose current traffic"
					   " is made of ~B vehicles, whose random state is ~w",
					   [ ?getAttr(name),
						 class_Actor:get_abstract_identifier( State ),
						 SourcePOIAAI, SourcePOIPid,
						 TargetPOIAAI, TargetPOIPid,
						 ?getAttr(length),
