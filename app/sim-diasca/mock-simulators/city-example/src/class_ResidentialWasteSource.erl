...
% Class modelling a residential waste source, which is a specific kind of waste
% loading point: it produces (residential) wastes spontaneously, in a stochastic
% manner.
%
...
-define(TraceEmitterCategorization,"City-example.Waste.Source.Residential").
...
construct( State, ActorSettings, Name, Location, ProductionType,
	...
	ActorState = class_StochasticActor:construct( State, ActorSettings, Name, [
	  { production_quantity_law, {gaussian,_Mu=ProductionQuantity,_Sigma=2.0} }
																 ] ),

	ProductionTickDuration = class_Actor:convert_seconds_to_ticks(
									  ProductionDuration, ActorState ),
	...
	WasteStockProbePid = class_Actor:declare_probe(
				_Name=io_lib:format( "~s Produced Waste Stock Probe",
									[ Name ] ),
				_Curves=[ io_lib:format( "Quantity of waste of type ~w "
										 "still in tank (in tons)",
										[ ProductionType ] ) ],
				_Zones=[],
				_Title=io_lib:format( "Waste Production & Storage Monitoring"
									  "for Residential Waste Source ~s",
									  [ Name ] ),
...
generate_definitions( ResidentialSourceCount, LocationGeneratorPid, GISPid ) ->
	...
	define_residential_waste_sources( ResidentialSourceCount, GISPid, _Acc=[] ).
...
define_residential_waste_sources( _ResidentialSourceCount=0, GISPid, Acc ) ->
...
define_residential_waste_sources( ResidentialSourceCount, GISPid, Acc ) ->
	...
	Name = io_lib:format( "ResidentialWasteSource-~B",
						 [ ResidentialSourceCount ] ),
	...
	define_residential_waste_sources( ResidentialSourceCount - 1, GISPid,
									NewAcc ).
...
merge_parameters( _Params=[ {  Name, ProductionType, ProductionQuantity,
	...
	NewResidentialSourceDef = { class_ResidentialWasteSource, [ Name,
		   { wgs84_cartesian, Loc }, ProductionType, ProductionQuantity,
						LocalStorage, ProductionDuration, GISPid ] },

	merge_parameters( Tp, Tl, [ NewResidentialSourceDef | Acc ], GISPid ).
...
to_string( State ) ->
	...
	io_lib:format( "Residential waste source '~s' located at ~s, "
				   "generating on average ~f tons of waste of type ~p "
				   ...
				   [ ?getAttr(name),
					 ...
					 ?getAttr(production_type),
					 text_utils:duration_to_string( 1000 * DurationInSeconds ),
