...
actSpontaneous( State ) ->
	...
	_CurrentTickOffset = ?getAttr(current_tick_offset),

	?wooper_return_state_only( State ).



% Returns the unsatisfied connections (if any) for that junction,
% i.e.:
%
...
% list of PID of the already-connected outbound POI)
%
...
generate_definitions( JunctionCount, LocationGeneratorPid, GISPid ) ->
	...
	define_junctions( JunctionCount, GISPid, _Acc=[] ).



define_junctions( _Junctioncount=0, GISPid, Acc ) ->
...
define_junctions( JunctionCount, GISPid, Acc ) ->

	% Defines the build parameters for a new junction; we want to end up with
	% a list of { class_RoadJunction,
	%   [ Name, Location, InboundCount, OutboundCount, GISPid ] } elements.

	Name = io_lib:format( "RoadJunction-~B", [ JunctionCount ] ),
	...
	define_junctions( JunctionCount - 1, GISPid, NewAcc ).



% Adds the location to the road build parameters (a kind of zip
% operation):
%
...
merge_parameters( _Params=[ { Name, InboundCount, OutboundCount } | Tp ],
	...
	merge_parameters( Tp, Tl, [ NewRoadDef | Acc ], GISPid ).
...
to_string( State ) ->
	...
	io_lib:format( "Road junction '~s' located at ~s, "
