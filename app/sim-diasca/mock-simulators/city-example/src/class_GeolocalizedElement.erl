...
construct( State, { wgs84_cartesian, CartesianCoord } ) ->
	setAttribute( State, location, CartesianCoord );
...
% (const request)
%
-spec getActualLocation( wooper_state() ) ->
...
% (oneway)
%
-spec setLocation( wooper_state(), class_GIS:geo_coordinate() ) ->
						 oneway_return().
...
enter_in( GeoContainerPid, State ) ->
	...
	receive
		...
		{ wooper_result, entered } ->
			setAttribute( State, location, GeoContainerPid );
...
% (helper)
%
-spec interpret_location( wooper_state() ) -> string().
interpret_location( State ) ->

	case ?getAttr(location) of

		Pid when is_pid(Pid) ->
			io_lib:format( "inside geocontained ~w", [ Pid ] );
