...
-define( wooper_construct_parameters, InitialDataSource ).
...
%
-spec construct( wooper_state(), data_source() ) -> wooper_state().
construct( State, 'none' ) ->
	construct_common( State );

construct( State, Filename ) when is_list(Filename) ->

	BasicState = construct_common( State ),

	case file_utils:is_existing_file( Filename ) of

		true ->
			InitialTable = getAttribute( BasicState, location_table ),
			NewLocationTable = add_locations_from( Filename, InitialTable ),
			setAttribute( BasicState, location_table, NewLocationTable ) ;

		false ->
			throw( { gis_data_file_not_found, Filename } )
...
%
-spec recordLocations( wooper_state(), geolocation_flavour(),
...
% (const request)
%
-spec resolveLocation( wooper_state(), location_name() ) ->
...
% (oneway)
%
-spec recordPointOfInterest( wooper_state(), poi_pid(), static_location() ) ->
								   oneway_return().
...
% (const request)
%
-spec searchNearestPointOfInterest( wooper_state(), poi_pid() ) ->
...
% (const request)
%
-spec searchNearestPointsOfInterest( wooper_state(), poi_pid(),
...
% (const request)
%
-spec searchNearestPointsOfInterest( wooper_state(), poi_pid(), [ poi_pid() ],
					basic_utils:count() ) -> request_return( [ poi_pid() ] ).
...
%
-spec declareRoad( wooper_state(), pid(), pid(), pid() ) ->
...
% (request, for synchronicity)
%
-spec declareRoads( wooper_state(),
...
% (request, for synchronicity)
%
-spec declarePOI( wooper_state(), pid() ) -> request_return( 'poi_declared' ).
...
% (request, for synchronicity)
%
-spec declarePOIs( wooper_state(), [ pid() ] ) ->
...
%
-spec render( wooper_state() ) -> request_return( 'gis_rendering_done' ).
render( State ) ->
...
% (const oneway)
%
-spec traceContent( wooper_state() ) -> oneway_return().
traceContent( State ) ->
...
toString( State ) ->
	...
	LocationStrings = [ io_lib:format( "~ts, at ~s", [ LocName,
	...
	FinalLocationString = case LocationStrings of
		...
		_ ->
			io_lib:format( "GIS recording following ~B locations:~s~n",
	...
	POIStrings = [ io_lib:format( "~w, at ~s", [POIPid ,
	...
	FinalPOIString = case POIStrings of
		...
		_ ->
			io_lib:format( "and recording following ~B points of interest:~s",
				[ length( POIStrings ),
				  text_utils:string_list_to_string( POIStrings ) ] )
...
wgs84_polar_to_string( { Latitude, Longitude, Elevation } ) ->
	io_lib:format( "WGS84 polar coordinates: latitude = ~f degrees, "
...
wgs84_cartesian_to_string( { X, Y, Z } ) ->
	io_lib:format( "WGS84 cartesian coordinates: X = ~f meters, "
...
shutdown( GISPid ) ->
	delete_synchronously_instance( GISPid ).
...
construct_common( State ) ->
	...
	TraceState = class_TraceEmitter:construct( State, "GIS service" ),
	...
	RoadNetwork = class_Mesh:new_link( _Name="Road Network 2D Topology",
									  _Opts=[] ),
