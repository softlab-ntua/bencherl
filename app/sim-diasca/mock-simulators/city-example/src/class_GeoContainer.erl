...
% (request)
%
-spec requestEntry( wooper_state() ) -> request_return( entry_outcome() ).
requestEntry( State ) ->
...
% (helper)
%
-spec request_entry( geolocalized_pid(), wooper_state() ) ->
						{ wooper_state(), entry_outcome() }.
...
to_string( State ) ->
	...
	ContainedString = case ?getAttr(contained) of
		  ...
		  List ->
				io_lib:format( "containing following ~B geo-elements: ~w",
							   [ length( List ), List ] )

	end,

	io_lib:format( "Geocontainer named '~s' located at ~w ~s",
