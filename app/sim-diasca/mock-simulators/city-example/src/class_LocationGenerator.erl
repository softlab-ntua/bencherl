...
% Class-specific attributes of a Location Generator instance are:
%
...
%
-spec construct( wooper:state(), class_TraceEmitter:name(), linear_3D:vector() )
			   -> wooper:state().
...
generateNonAdjacentLocation( State, Locations, Radius ) ->
	...
	?wooper_return_state_result(
	   appendToAttribute( State, past_locations, Loc ), Loc ).
