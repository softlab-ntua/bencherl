...
onFirstDiasca( State, _SendingActorPid ) ->
	...
	% A truck starts empty and with no specific intent:
	ChoiceState = apply_default_behaviour( ProbeState ),

	?wooper_return_state_only( ChoiceState ).
...
% (actor oneway)
%
-spec notifyNoLoadedWaste( wooper_state(), pid() ) ->
...
% (actor oneway)
%
-spec notifyLoadedWaste( wooper_state(), unit_utils:tons(), waste_type(),
...
% (actor oneway)
%
-spec notifyNoUnloadedWaste( wooper_state(), pid() ) ->
								 class_Actor:actor_oneway_return().
...
% (actor oneway)
%
-spec notifyUnloadedWaste( wooper_state(), unit_utils:tons(), waste_type(),
			class_TimeManager:tick_offset(), loading_point_pid() ) ->
							   class_Actor:actor_oneway_return().
...
% (actor oneway)
%
-spec notifyEntryOutcome( wooper_state(), entry_outcome(), poi_type(),
...
% (actor oneway)
%
-spec notifyPlannedRoadExit( wooper_state(), class_TimeManager:tick_offset(),
...
% (actor oneway)
%
-spec notifyRoadExit( wooper_state(), poi_pid(), road_pid() ) ->
					class_Actor:actor_oneway_return().
...
generate_definitions( WasteTruckCount, AllPOIS, POICount, Acc ) ->
	...
	TruckName = io_lib:format( "WasteTruck-~B", [ WasteTruckCount ] ),
...
% (helper)
%
-spec record_intents( intent() | [ intent() ], wooper_state() ) ->
							wooper_state().
...
% (helper)
%
-spec apply_intents( intent() | [ intent() ], wooper_state() ) ->
						   wooper_state().
...
% (helper)
%
-spec apply_behaviour( wooper_state() ) -> wooper_state().
apply_behaviour( State ) ->
...
% (helper)
%
-spec plan_next_loading( wooper_state() ) -> intent().
plan_next_loading( State ) ->
...
% (helper)
%
-spec plan_next_unloading( wooper_state() ) -> intent().
plan_next_unloading( State ) ->
...
% (helper)
%
-spec attempt_loading( wooper_state() ) -> wooper_state().
...
% (helper)
%
-spec attempt_unloading( wooper_state() ) -> wooper_state().
...
% (helper)
%
-spec drive( path(), wooper_state() ) -> wooper_state().
...
% (helper)
%
-spec apply_default_behaviour( wooper_state() ) -> wooper_state().
apply_default_behaviour( State ) ->
...
% (helper)
%
-spec apply_opportunistic_behaviour( wooper_state() ) -> wooper_state().
...
find_path_to_loading_point( State ) ->
	...
	% Tells whether the place in parameter is suitable for the loading of
	...
	LoadablePredicate = fun( POIPid, _UserData ) ->
								...
								case POIPid of
									...
									_ ->
										is_poi_loading_compatible( POIPid,
														  CurrentWasteType )
...
% (const helper)
%
-spec is_full( wooper_state() ) -> 'true'
...
% (const helper)
%
-spec is_empty( wooper_state() ) -> 'true'
				  | { waste_type(), unit_utils:tons() }.
is_empty( State ) ->
...
% (helper)
%
-spec get_waste_type( wooper_state() ) -> 'none' | waste_type().
...
% (helper)
%
-spec get_remaining_free_mass( wooper_state() ) -> unit_utils:tons().
...
to_string( State ) ->
	...
	io_lib:format( "Waste truck '~s' in location ~w, storing ~s; "
				   "current intents: ~p",
			[ ?getAttr(name), ?getAttr(location),
