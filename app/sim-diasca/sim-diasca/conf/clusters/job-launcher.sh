...
Launches the specified executable with specified parameters on the cluster, through the PBS-based job manager."

# Note: this script interacts with the job manager, and is not Sim-Diasca
# specific per se.


# This script typically outputs on the console something like:
...
if [ ! -x "$qsub_cmd" ] ; then
	...
	echo "  Error, qsub executable not found." 1>&2
	exit 35
...
# output:
qsub_output_file="/tmp/.sim-diasca-qsub-by-$USER-on-"`date '+%Y%m%d-%Hh%Mm%Ss'`"-$$.txt"

[ $do_debug -eq 1 ] || echo "Qsub command: $qsub_cmd ${all_parameters} 1>$qsub_output_file 2>&1"
