...
%
-spec construct( wooper_state(), file_utils:file_name(), [ filter_spec() ],
				[ filter_spec() ], [ filter_spec() ] ) -> wooper_state().
...
% Extracts the information for all curves, returns a
% {CurveMetadataList,NextLines} pair where CurveMetadataList is a list of
% curve_metadata records.
%
...
update_curves( _CurvesMetadata=[ C=#curve_metadata{
		   ...
		   tick_count=TickCount } | Curves ], _Values=[V|T], Tick, CurveAcc ) ->
...
% specified parameters.
%
create_curve_filter( _FilterSpec={FilterName,FilterParameters}, CurveName ) ->
