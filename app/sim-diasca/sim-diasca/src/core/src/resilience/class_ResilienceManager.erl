...
-spec construct( wooper_state(),
				 ...
				 [ computing_host_info() ] ) -> wooper_state().
...
% (request)
%
-spec triggerSerialisation( wooper_state(), class_TimeManager:tick_offset(),
...
relink_instances( SurvivorAgentPidList, State ) ->
	...
	case wooper:send_and_wait_request( _Req=relinkInstances, _Args=[],
...
-spec trigger_serialisation( class_TimeManager:tick_offset(),
	   class_TimeManager:diasca(), wooper_state() ) -> wooper_state().
trigger_serialisation( Tick, Diasca, State ) ->
	...
	%
	case wooper:send_and_wait_request( _Req=serialiseNode,
