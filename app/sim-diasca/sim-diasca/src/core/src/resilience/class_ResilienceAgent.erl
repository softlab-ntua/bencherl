...
%
-spec construct( wooper_state(), pid(), [ node_name() ], [ node_name() ],
				text_utils:bin_string() ) ->  wooper_state().
...
% (request)
%
-spec serialiseNode( wooper_state(), class_TimeManager:tick_offset(),
...
% (request)
%
-spec notifyOfLocalProbes( wooper_state(), [ class_Probe:probe_pid() ] ) ->
...
% (request)
%
-spec recoverNodes( wooper_state(), [ net_utils:atom_node_name() ],
...
% (request)
%
-spec relinkInstances( wooper_state() ) ->
...
% (oneway)
%
-spec updateResilienceMapping( wooper_state(), [ net_utils:atom_node_name() ],
