...
%
-spec construct( wooper:state(), [ file_utils:directory_name() ] ) ->
					   wooper:state().
...
%
-spec notifyEvent( wooper:state(), sim_diasca_plugin:plugin_event() ) ->
...
% (request, for synchronicity)
%
-spec notifyParametrisedEvent( wooper:state(),
...
%
-spec get_plugins_from( [ file_utils:directory_name() ], wooper:state() ) ->
							  [ string() ].
