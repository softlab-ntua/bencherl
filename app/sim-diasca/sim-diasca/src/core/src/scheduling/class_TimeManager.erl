...
-type simulation_events() :: 'simulation_started'   | 'simulation_suspended'
						   | 'simulation_resumed' | 'simulation_succeeded'
						   | 'simulation_stopped'.
...
% the simulation stalled and notify the user about it.
%
...
% 45 seconds:
-define( watchdog_wait_duration, 45000 ).
...
% 15 seconds:
-define( watchdog_wait_duration, 15000 ).
...
%
% More precisely, for each tick and non-null diasca, we first add the number of
...
%
% - diasca_count :: 'undefined' | basic_utils:count() keeps track of the number
% of all diascas evaluated up to now, for example to measure concurrency
%
% - schedule_count :: 'undefined' | basic_utils:count() keeps track of the total
% number of schedulings across all diascas, notably to be able to compute the
...
% is started.
%
-spec construct( wooper:state(), virtual_seconds(), interactivity_mode(),
...
% (request, for synchronisation purposes)
%
-spec declareChildManager( wooper:state() ) -> request_return(
...
%
-spec registerBootstrapScheduling( wooper:state(), pid() ) ->
										 request_return( 'load_balancer_set' ).
...
% (oneway)
%
-spec setInitialSimulationDate( wooper:state(), unit_utils:date(),
...
% (oneway)
%
-spec setFinalSimulationDate( wooper:state(), unit_utils:date(),
		 unit_utils:time() ) -> oneway_return().
...
% (oneway)
%
-spec start( wooper:state(), tick_offset() ) -> oneway_return().
...
% (oneway)
%
-spec start( wooper:state(), tick_offset(), pid() ) -> oneway_return().
...
% (oneway)
%
-spec startFor( wooper:state(), number() ) -> oneway_return().
...
% (oneway)
%
-spec startFor( wooper:state(), number(), pid() ) -> oneway_return().
...
% (request)
%
-spec stop( wooper:state() ) -> request_return( {'stopped',pid()} ).
stop( State ) ->
	...
	StoppedState = case ?getAttr(started) of
		...
		true ->
			...
			?display_console( "Stopping at #~p, hence deleting actors ~p.~n",
...
% (const oneway)
%
-spec resume( wooper:state() ) -> oneway_return().
resume( State ) ->
...
% (oneway)
%
-spec addSimulationListener( wooper:state(), pid() ) -> oneway_return().
...
% (oneway)
%
-spec removeSimulationListener( wooper:state(), pid() ) -> oneway_return().
...
% (oneway)
%
-spec addTimeListener( wooper:state(), pid() ) -> oneway_return().
...
% (oneway)
%
-spec removeTimeListener( wooper:state(), pid() ) -> oneway_return().
...
% (const request)
%
-spec getAllLocalActors( wooper:state() ) -> request_return( [ pid() ] ).
...
beginTimeManagerTick( State, NewTickOffset ) ->
	...
	?debug_fmt( "beginTimeManagerTick: new tick offset is #~B, "
	...
	% '?info(..)' so that the traces are output with a timestamp set to the new
	% current tick rather than the previous one still in
	...
	FinalState = case NewTickOffset of
		...
		LateOffset when LateOffset >= StopTickOffset ->
			...
			{ StoppedState, { stopped, _SelfPid } } = executeRequest(
														NewState, stop ),
			StoppedState;
		...
		_NonTerminalOrUndefinedStopTickOffset ->
			...
			?display_console( "beginTimeManagerTick: continuing at tick #~B.~n",
							  [ NewTickOffset ] ),
...
% (oneway)
%
-spec notifySpontaneousSubtreeCompletion( wooper:state(), tick_offset(), pid(),
...
notifySpontaneousSubtreeCompletion( State, SubtreeTickOffset, ChildManagerPid,
	...
	?debug_fmt( "Time manager ~w received notifySpontaneousSubtreeCompletion "
...
% (oneway)
%
-spec notifySpontaneousActionsCompleted( wooper:state(), tick_offset(), pid(),
...
notifySpontaneousActionsCompleted( State, ActorTickOffset, ActorPid,
	...
	?debug_fmt( "Time manager ~w received notifySpontaneousActionsCompleted "
...
notifySpontaneousWatchdogCompleted( State, WatchdogTickOffset ) ->
	...
	?debug_fmt( "Time manager ~w received notifySpontaneousWatchdogCompleted "
...
beginTimeManagerDiasca( State, TickOffset, NewDiasca ) ->
	...
	?debug_fmt( "beginTimeManagerDiasca: at tick offset #~B, "
...
% (const request)
%
-spec getSimulationTick( wooper:state() ) -> request_return( tick() ).
...
% (const request)
%
-spec convertSecondsToTicks( wooper:state(),
...
% (const request)
%
-spec convertSecondsToNonNullTickDuration( wooper:state(),
		  unit_utils:seconds() | virtual_seconds() ) ->
											 request_return( tick_offset() ).
...
scheduleTrigger( State, TriggerTickOffset, TriggerDiasca ) ->
	...
	?display_console( "scheduleTrigger: ~w adding actor to trigger ~w "
					  ...
					  "at diasca ~B of tick offset #~B.~n",
...
% (oneway)
%
-spec notifyNudged( wooper:state(), pid(), tick_offset(), [pid()] ) ->
						  oneway_return().
...
% (oneway)
%
-spec onWooperExitReceived( wooper:state(), pid(), basic_utils:exit_reason() )
						  -> oneway_return().
...
% (oneway)
%
-spec onWallclockMilestone( wooper:state(), unit_utils:milliseconds() ) ->
								  oneway_return().
onWallclockMilestone( State, CurrentMillisecond ) ->
	...
	?display_console( "Wall-clock milestone triggered on ~p (~p) "
...
% (const request)
%
-spec getProgressDiagnosis( wooper:state() ) -> request_return(
...
% (oneway)
%
-spec declareDataExchanger( wooper:state(), pid() ) -> oneway_return().
...
% (helper)
%
-spec display_timing_information( string(), wooper:state() ) ->
										basic_utils:void().
display_timing_information( Timings, State ) ->
	...
	io:format( "Simulation terminated successfully at ~s, "
			   "after a duration of ~s in simulation time (~B ticks),"
...
% (helper)
%
-spec display_concurrency_information( wooper:state() ) -> basic_utils:void().
	...
	case ?getAttr(diasca_count) of
		...
		DiascaCount ->
			...
			io:format( "In the course of this run, a total of "
					   ...
					   "diasca (not counting any engine-level "
					   "activity).~n~n",
...
% (oneway)
%
-spec notifyOverallActorCount( wooper:state(), basic_utils:count() ) ->
									oneway_return().
...
timer_main_loop( TimeManagerPid, TimeOut ) ->
	...
	receive
		...
		delete ->
			?display_console( "Timer: timer_main_loop requested to stop.~n",
							  [] ),
...
watchdog_main_loop( TimeManagerPid, Period, AccumulatedDuration, MaxIdle ) ->
	...
	?display_console( "Watchdog ~w running (period: ~B ms, "
					  "accumulated: ~B ms).~n",
	...
	receive
		...
		{ beginWatchdogDiasca, [ TickOffset, NewDiasca ] } ->
			...
			?display_console( "Watchdog received a notification of diasca "
							  "begin for tick offset #~B diasca ~B.~n",
								[ TickOffset, NewDiasca ] ),
		...
		timeManagerShutdown  ->
			?display_console( "Watchdog shut down.~n", [] ),
			?notify_debug( "Watchdog removed (shutdown)." );


		delete ->
			?display_console( "Watchdog deleted.~n", [] ),
			?notify_debug( "Watchdog removed (deletion)." );


		{ synchronous_delete, ListenerPid } ->
			?display_console( "Watchdog synchronously deleted.~n", [] ),
	...
	after Period ->
		...
		case AccumulatedDuration of
			...
			TooLong when TooLong > MaxIdle ->
				...
				?display_console( "Warning: watchdog on node '~s' detected "
	...
	after 0 ->
			...
			case LatestMeasuredSecond of
				...
				LatestDisplayedSecond ->
					...
					%?display_console( "(still in second ~B).~n",
					%				  [ LatestSecond ] ),
					timer:sleep( 200 ),
				...
				NewSecond ->
					%?display_console( "latest = ~w, current = ~w.~n",
...
display_console_line( _Timings={ SimDateString, SimTimeString,
	...
	io:format( "~sA: ~11.B|S: ~11.B|P: ~12.B |~n",
...
%
-spec get_current_tick_offset( wooper:state() ) -> tick_offset() | 'undefined'.
...
get_current_tick( State ) ->
	%?display_console( "get_current_tick called.~n" ),
...
% (helper)
%
-spec get_simulation_time_and_date( wooper:state() ) -> basic_utils:timestamp().
...
% (helper)
%
-spec timestamp_to_ticks( basic_utils:timestamp(), wooper:state() ) ->
								tick().
...
%
-spec convert_seconds_to_ticks( virtual_seconds() | unit_utils:seconds(),
			  math_utils:percent(), wooper:state() ) -> tick_offset().
...
init( State ) ->
	...
	StopString = case ?getAttr(stop_tick_offset) of
					 ...
					 StopOffset ->
						 ...
						 io_lib:format( "Simulation will stop no later than "
										"tick ~B (i.e. after ~B ticks, "
										"which is a duration of ~s in "
										"virtual time).",
	...
	%
	?display_console( "Simulation started at ~s with a simulation frequency "
...
start_actors_by_chunks( InitialActors, StartMessage ) ->
	...
	?display_console( "start_actors_by_chunks for ~B actors.~n",
					  [ ?list_impl:size( InitialActors ) ] ),
...
wait_for_start_acknowlegments( WaitedType, WaitedList ) ->
	...
	?display_console( "wait_for_start_acknowlegments for ~B waited "
					  "processes.~n", [ ?list_impl:size( WaitedList ) ] ),
...
manage_possible_end_of_diasca( State ) ->
	...
	case is_current_diasca_over( State ) of
		...
		false ->
			...
			?display_console( "Time manager ~w still having to wait.~n", [ self() ] ),
	...
	case ?getAttr(next_action) of
		...
		new_diasca_needed ->
			...
			?display_console( "Root time manager creates a new diasca, "
							  "{~B,~B}.~n", [ CurrentTickOffset, NewDiasca ] ),
	...
	case ?getAttr(interactivity_mode) of
		...
		batch ->
			...
			?display_console( "Root time Manager ~w at tick offset #~B "
							  "determined that the next tick should be #~p.~n",
...
wait_for_serialisation_end( _ActorsReturned=true, _Serialised=true, State ) ->
	...
	%
	?display_console( "Waiting for serialisation_done.~n", [] ),

	receive

		{ wooper_result, serialisation_done } ->

			?display_console( "Serialisation over, resuming simulation.~n",
							  [] ),
...
check_waited_count_consistency( State ) ->
	...
	?display_console( "~w waiting for C=~B, S=~B, T=~B, W=~B.~n", [ self(),
...
%
-spec display_waiting_reason( wooper:state() ) -> basic_utils:void().
display_waiting_reason( State ) ->
...
manage_new_tick( NewTickOffset, State ) ->
	...
	?display_console( "~n-- scheduling new tick, #~B (hence diasca 0), on ~w~n",
					  [ NewTickOffset, self() ] ),
	...
	?display_console( "~n~s for ~w: ~s is sent to ~B child time manager(s) "
...
manage_new_diasca( TickOffset, NewDiasca, State ) ->
	...
	?display_console( "~n~s for ~w: ~s is sent to ~B child time manager(s) "
					  ...
					  "(waited count, including any watchdog: ~B).~n",
	...
	KnownNextAction = case ?list_impl:is_empty( EarlyActors ) of
			 ...
			 false ->
				  ...
				  ?display_console( "Early actor detected, "
									"new diasca needed!~n", [] ),
	...
	case get_spontaneous_for( NewTickOffset, Agenda ) of
		...
		{ ActorListImpl, NewSpontaneousAgenda } ->
			...
			?display_console( "Notifying at tick offset #~B "
							  ...
							  [ NewTickOffset,
								?list_impl:to_list( ActorListImpl ) ] ),
...
terminate_actors( TickOffset, NewDiasca, State ) ->
	...
	?display_console( "Terminated actors: ~w.~n", [ ActorList ] ),
...
actual_unsubscribing( ActorPid, State ) ->
	...
	?display_console( "############ Unsubscribing actor ~w.~n", [ ActorPid ] ),
...
pre_serialise_hook( State ) ->
	...
	%
	wooper_serialisation:mute_attributes( [
...
% manager.
%
-spec merge_local_with( wooper_serialisation:term_serialisation() ) ->
							  basic_utils:void().
