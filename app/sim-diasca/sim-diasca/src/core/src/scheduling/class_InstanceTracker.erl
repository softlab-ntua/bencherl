...
% just happens that it is the single one who does not have a parent tracker.
...
% Description of all the state attributes of an instance tracker instance:
%
...
% 'undefined')
%
% - child_trackers :: [ { pid(), node() } ] is a list of the { TrackerPid,
% TrackerNode } pairs for all the (direct) child trackers (if any) of this
% tracker
%
...
% enabled
%
-spec construct( wooper_state(), pid() | 'none', boolean() ) -> wooper_state().
...
delete( State ) ->
	...
	delete_synchronously_instances( ?getAttr(child_trackers) ),
...
%
-spec declareTrackers( wooper_state(), [ pid() ] ) ->
...
% (oneway)
%
-spec registerActor( wooper_state(), class_Actor:aai(),
...
% (oneway)
%
-spec registerActor( wooper_state(), pid(), actor_info() ) -> oneway_return().
registerActor( State, ActorPid, ActorInfo ) ->
...
% (oneway)
%
-spec unregisterActor( wooper_state(), pid(), class_name() ) -> oneway_return().
...
%
-spec registerAgent( wooper_state(), wooper:class_name() ) ->
...
% (request, for synchronisation purposes)
%
-spec registerThirdPartyAgent( wooper_state(), wooper:class_name(), pid() ) ->
						   request_return( 'agent_registered' ).
...
% (oneway)
%
-spec unregisterAgent( wooper_state(), pid() ) -> oneway_return().
...
% (request, for synchronisation purposes)
%
-spec registerResultProducer( wooper_state(), producer_ref() ) ->
...
% (oneway)
%
-spec unregisterResultProducer( wooper_state(), pid() ) -> oneway_return().
...
% (const request)
%
-spec getActorInformationLocal( wooper_state(), pid() ) -> request_return(
...
% (const request)
%
-spec getActorInformationGlobal( wooper_state(), pid() ) ->
...
getActorInformationGlobal( State, ActorPid ) ->
	...
	ActorInfo = case LocalNode of
		...
		_OtherNode ->
			...
			% Finds the tracker in charge of the actor node:
			{ TrackerPid, _SameOtherNode } = lists:keyfind( _Key=ActorNode,
								   _Index=2, ?getAttr(child_trackers) ),
...
% request.
%
% So the caller shall only expect in return a { notifyResolvedPid, Res }
% message, where Res :: class_Actor:aai() | agent_ref() | producer_ref
% message(), which may be interpreted as a oneway.
%
-spec resolvePid( wooper_state(), pid(), pid() ) -> oneway_return().
...
% (const oneway)
%
-spec resolveLocalPid( wooper_state(), pid(), pid() ) -> oneway_return().
...
%
-spec setCrashedNodeMapping( wooper_state(), ?hashtable_type:?hashtable_type() )
...
setCrashedNodeMapping( State, NodeTable ) ->
	...
	% Go recursive first:
	wooper:send_request( _RequestName=setCrashedNodeMapping,
...
% (const request)
%
-spec getStaticResourceInformation( wooper_state() ) ->
	 request_return(
...
% (const request)
%
-spec getDynamicResourceInformation( wooper_state() ) ->
...
% (helper)
%
-spec register_agent( wooper:class_name() | wooper_state() ) ->
							basic_utils:void().
...
% (helper)
%
-spec get_local_actor_info( pid(), wooper_state() ) -> actor_info().
get_local_actor_info( ActorPid, State ) ->
