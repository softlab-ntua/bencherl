...
% acotr; it will then be scheduled only once
%
...
%
% - simulation_tick_duration :: class_TimeManager:virtual_seconds is the actual
% duration, in floating-point milliseconds (in virtual time), between two
% simulation ticks
...
%
-spec construct( wooper_state(), actor_settings(), name() ) -> wooper_state().
construct( State,
...
% (request)
%
-spec simulationStarted( wooper_state(), class_TimeManager:tick(),
...
% (oneway)
%
-spec beginTick( wooper_state(), class_TimeManager:tick_offset() ) ->
								  oneway_return().
...
% (oneway)
%
-spec beginDiasca( wooper_state(), class_TimeManager:tick_offset(),
...
% (oneway)
%
-spec beginTerminationDiasca( wooper_state(), class_TimeManager:tick_offset(),
	class_TimeManager:diasca() ) -> oneway_return().
...
% (oneway)
%
-spec scheduleNextSpontaneousTick( wooper_state() ) -> oneway_return().
...
% (oneway)
%
-spec addSpontaneousTick( wooper_state(), class_TimeManager:tick_offset() ) ->
								oneway_return().
...
% (oneway)
%
-spec addSpontaneousTicks( wooper_state(),
...
% (oneway)
%
-spec withdrawSpontaneousTick( wooper_state(),
						class_TimeManager:tick_offset() ) -> oneway_return().
...
% (oneway)
%
-spec withdrawSpontaneousTicks( wooper_state(),
			[ class_TimeManager:tick_offset() ] ) -> oneway_return().
...
% (oneway)
%
-spec declareTermination( wooper_state() ) -> oneway_return().
...
% (oneway)
%
-spec declareTermination( wooper_state(), termination_delay() ) ->
								oneway_return().
...
% (helper)
%
-spec declare_termination( termination_delay(), wooper_state() ) ->
								 wooper_state().
...
% (oneway)
%
-spec receiveActorMessage( wooper_state(), class_TimeManager:tick_offset(),
...
% (oneway)
%
-spec acknowledgeMessage( wooper_state(), pid() ) -> oneway_return().
...
% (const request)
%
-spec getSimulationTick( wooper_state() ) ->
...
% (const request)
%
-spec getHostingNode( wooper_state() ) ->
...
% (const request)
%
-spec convertSecondsToTicks( wooper_state(), unit_utils:seconds() ) ->
		   request_return( class_TimeManager:tick_offset() ).
...
% (const request)
%
-spec convertTicksToSeconds( wooper_state(), class_TimeManager:tick_offset() )
...
% (const oneway)
%
-spec nudge( wooper_state(), pid() ) -> oneway_return().
...
% (const request)
%
-spec getAAI( wooper_state() ) -> request_return( aai() ).
getAAI( State ) ->
...
% (const request)
%
-spec getActorInfo( wooper_state() ) -> request_return( actor_info() ).
getActorInfo( State ) ->
...
% (actor oneway)
%
-spec onActorCreated( wooper_state(), pid(), class_name(),
...
% (const oneway)
%
-spec triggerNameNotification( wooper_state(), pid() ) -> oneway_return().
...
% (helper function)
%
-spec get_abstract_identifier( wooper_state() ) -> aai().
...
% (helper function)
%
-spec get_deployed_root_directory( wooper_state() ) -> string().
...
% (helper function)
%
-spec convert_seconds_to_ticks( number(), wooper_state() ) ->
...
% (helper function)
%
-spec convert_seconds_to_ticks( number(), math_utils:percent(), wooper_state() )
							  -> class_TimeManager:tick_offset().
...
% (helper function)
%
-spec convert_seconds_to_non_null_ticks( number(), wooper_state() ) ->
									class_TimeManager:tick_offset().
...
% (helper function)
%
-spec get_current_tick_offset( wooper_state() ) ->
								class_TimeManager:tick_offset().
...
% is hidden).
%
-spec get_current_tick( wooper_state() ) -> class_TimeManager:tick().
...
% (helper function)
%
-spec send_actor_message( pid(), oneway_call(), wooper_state() ) ->
								wooper_state().
...
-spec send_delayed_actor_message( class_TimeManager:tick_offset(), pid(),
				  oneway_call(), wooper_state() ) -> wooper_state().
...
% (helper function)
%
-spec send_actor_messages( [ pid() ], oneway_call(), wooper_state() ) ->
								wooper_state().
...
% (const helper)
%
-spec get_name( wooper_state() ) -> class_Actor:internal_name().
get_name( State ) ->
	?getAttr(name).
...
% (const helper)
%
-spec is_running( wooper_state() ) -> boolean().
is_running( State ) ->
...
% (helper function)
%
-spec create_actor( class_name(), [ method_argument() ], wooper_state() ) ->
						wooper_state().
...
-spec create_placed_actor( class_name(), [ method_argument() ],
	  class_LoadBalancer:placement_hint(), wooper_state() ) -> wooper_state().
...
% (helper function)
%
-spec enable_data_exchange( wooper_state() ) -> wooper_state().
...
%
-spec define_data( class_DataExchanger:entries(), wooper_state() ) ->
						basic_utils:void().
...
%
-spec define_data( class_DataExchanger:key(), class_DataExchanger:value(),
	  class_DataExchanger:qualifier(), wooper_state() ) -> basic_utils:void().
...
%
-spec modify_data( class_DataExchanger:entries(), wooper_state() ) ->
		basic_utils:void().
...
% (helper function)
%
-spec read_data( class_DataExchanger:key(), wooper_state() ) ->
...
%
% (helper function)
-spec read_qualified_data( class_DataExchanger:key(), wooper_state() ) ->
