...
% (helper)
%
-spec update_plan( class_TimeManager:tick_offset(), wooper_state() ) ->
						wooper_state().
...
% (helper)
%
-spec terminate( class_TimeManager:tick_offset(), wooper_state() ) ->
						wooper_state().
...
% (helper)
%
-spec behave_normally( class_TimeManager:tick_offset(), wooper_state() ) ->
						wooper_state().
...
-spec determine_next_spontaneous_tick( class_TimeManager:tick_offset(),
			wooper_state() ) -> 'undefined' | class_TimeManager:tick_offset().
...
% (helper)
%
-spec ensure_planned( class_TimeManager:tick_offset(), wooper_state() ) ->
...
% (helper)
%
-spec lowerTraceIntensity( wooper_state() ) -> wooper_state().
...
% (actor oneway)
%
-spec onActorCreated( wooper_state(),
	   { pid(), class_name(), [method_argument()] }, pid() ) -> wooper_state().
...
%
-spec addInitialPeer( wooper_state(), pid() ) -> request_return( 'peer_added' ).
...
% (actor oneway)
%
-spec addPeer( wooper_state(), pid() ) -> oneway_return().
...
% (actor oneway)
%
-spec removePeer( wooper_state(), pid() ) -> oneway_return().
...
% (actor oneway)
%
-spec forgetPeer( wooper_state(), pid() ) -> oneway_return().
...
% (oneway)
%
-spec hello( wooper_state(), class_Actor:internal_name(), pid() ) ->
				   oneway_return().
