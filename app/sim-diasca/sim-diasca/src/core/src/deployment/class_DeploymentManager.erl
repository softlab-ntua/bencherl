...
%
-spec construct( wooper_state(), simulation_settings(), deployment_settings(),
	load_balancing_settings(), simulation_context() ) -> wooper_state().
construct( State,
	...
	Cookie = list_to_atom( basic_utils:generate_uuid() ),
	...
	UserSettings = { SimulationName, _InteractivityMode, _TickDuration,
					 ...
					 DeploymentSettings, _PlacementPolicy,
	...
	?send_info_fmt( StartingState,
			 "Creating a new deployment manager with simulation settings:~s "
			 "with deployment settings:~s and with load balancing settings:~s"
			 "~nVersions are:~n"
...
construct( State, SimulationSettings, DeploymentSettings, LoadBalancingSettings,
	...
	set_up_simulation_services( UserSettings, RuntimeSettings,
											Context, SetState ).
...
set_up_simulation_services(
	...
	RootDataExchangerPid = set_up_data_exchanging( DataExchangeSettings,
						RootTimeManagerPid, BaseNodeName, ServicePlacement ),
...
delete( State ) ->
	...
	% services, if running:
	%
	FirstDeletedState = delete_synchronously_any_instance_referenced_in(
		[ performance_tracker_pid, root_data_exchanger_pid,
		 resilience_manager_pid ], State ),

	SecondDeletedState = delete_synchronously_any_instance_referenced_in(
		[ root_time_manager_pid, result_manager_pid ], FirstDeletedState ),

	class_PluginManager:notify( on_simulator_stop ),

	ThirdDeletedState = delete_synchronously_any_instance_referenced_in(
	...
	FourthDeletedState = delete_synchronously_any_instance_referenced_in(
	...
	delete_synchronously_instances( get_host_managers( FifthDeletedState ) ),
...
% (helper)
%
-spec process_setup_outcome( file_utils:file_name(), wooper_state() ) ->
	{ [ computing_host_info() ], [ { pid(), atom() } ], wooper_state() }.
...
% (const pseudo oneway)
%
-spec onHostDeploymentFailure( wooper_state(), pid(), term() ) ->
									 oneway_return().
...
% (const request)
%
-spec getLoadBalancer( wooper_state() ) -> request_return( pid() ).
...
% (const request)
%
-spec getRootTimeManager( wooper_state() ) -> request_return( pid() ).
...
% (const request)
%
-spec getComputingNodes( wooper_state() ) ->
...
% (request)
%
-spec activateDatabase( wooper_state() ) ->
...
% (request)
%
-spec deactivateDatabase( wooper_state() ) ->
...
% (oneway)
%
-spec notifyResilienceAgents( wooper_state(), [ pid() ] ) ->
									oneway_return().
	...
	PackageString = case PackageManager of
		...
		generate_deployment_package ->
			"the simulation deployment package will be generated on-the-fly "
				"and saved in a deployment archive file, "
				++ RebuildString;
...
determine_root_directory() ->
	...
	%
	file_utils:join( [ CurrentDir, RelativeRootDir, ".." ] ).
...
% (helper)
%
-spec get_host_info( pid(), wooper_state() ) -> computing_host_info().
...
% (helper)
%
-spec get_hostname_for( pid(), wooper_state() ) -> binary().
...
% (helper)
%
-spec get_username_for( pid(), wooper_state() ) -> binary().
...
apply_configuration_changes( _ConfChanges=#configuration_changes{
				compute_scheduler_count=ComputeSchedulerCount
											 }, State ) ->
...
determine_user_settings( SimulationSettings, DeploymentSettings,
	...
	{ SimulationName, InteractivityMode, TickDuration, EvaluationMode,
	  ...
	  DataExchangerSettings, PlacementPolicy, ResultSpecification,
	  ResilienceLevel, FullSettings }.
...
check_placement_policy( #load_balancing_settings{
	...
	throw( { invalid_placemment_policy, Other } ).
...
set_up_result_management_and_datalogging( SimulationName, StartTimestamp,
	...
	RootTimeManagerPid, _DataLoggerWanted,
	...
	ResultManagerPid = class_ResultManager:remote_synchronous_timed_new_link(
		ResultManagerNode, ResultSpecification, RootTimeManagerPid, RunDir,
		ResultDir, ResultMetadata ),
	...
	% We create it unconditionally (regardless of DataLoggerWanted), as the
	% actual database operations are triggered only when actually used:
	DataLoggerPid = class_DataLogger:create_main_datalogger( DataLoggerNode,
													ResultMetadata ),
...
set_up_load_balancing( PlacementPolicy, SelectedNodes,
		...
		RootTimeManagerPid, AllInstanceTrackers,
	...
	basic_utils:wait_for( _Message={ wooper_result, load_balancer_set },
						  _Count=length( AllInstanceTrackers ) + 1 ),
...
set_up_data_exchanging( #deployment_settings{ enable_data_exchanger=false },
				   _RootTimeManagerPid, _BaseNodeName, _ServicePlacement ) ->
	undefined.
...
% (helper)
%
-spec manage_simulation_package( wooper_state() ) -> file_utils:file_name().
...
build_simulation_package( State ) ->
	...
	ActualAdditions = get_engine_deployment_settings(
	DeploySettings#deployment_settings.rebuild_on_deployment_package_generation)
		++ AdditionalElemList ++ ConfigurationFiles,
...
rebuild_directory( Directory, State ) ->
	...
	case os:cmd( Command ) of
		...
		"make_failed\n" ->
			...
			% only the error output: (error messages are not always as
			% explanatory as expected).
...
reorder_nodes( _NodeList=[ E={ _, LastHost } | T ], LastHost, Acc ) ->
