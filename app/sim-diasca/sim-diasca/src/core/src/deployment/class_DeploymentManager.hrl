...
-record( deployment_settings, {
	...
	% "/home/boudevil/My-data/foo.dat"), or a path relative to the root of the
	% Sim-Diasca installation (ex: "data/bootstrap-infos"; this will expect the
	% 'data' directory to be in the same directory as 'common', 'sim-diasca',
	% 'wooper', etc.); if a specified path refers to a file, that file will be
	% added to the deployment archive. If a specified path refers to a
	% directory, then it will be recursively scanned at archive creation, in
	% order to look-up the content to integrate to the archive
	%
	...
	%     list of suffixes that will be black-listed (filenames ending with them
	%     will be rejected); for example
	%     { exclude_suffixes, [ "_test.beam", "-local.dat", ".png" ] }
	%
	...
	% (relative to the root of the deployment archive) to the configuration
	% files that must be read from the deployment archive and parsed.
	%
