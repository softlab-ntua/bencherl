...
-define( wooper_construct_parameters, ResultSpecification, DataLoggerEnabled,
		 RootTimeManagerPid, SimRunDir, ResultDirName, Metadata ).
...
% the result queue which is in charge of it.
...
% list are { BinaryPattern, PrecompiledMatchSpec, OptionList } triplets,
...
% (oneway)
%
-spec setResourceMapping( wooper:state(), [ { net_utils:atom_host_name(),
...
% (request)
%
-spec declareProbe( wooper:state(), binary(), boolean() ) ->
...
declareProbe( State, BinProbeName, IsToBeTracked ) ->
	...
	case is_result_wanted( BinProbeName, basic_probe, State ) of
		...
		{ true, ProbeOptions } ->
			...
			% First, check the same probe will not be registered twice:
...
% (const request)
%
-spec isResultProducerWanted( wooper:state(), text_utils:bin_string() ) ->
...
% (const request)
%
-spec isResultProducerWantedWithOptions( wooper:state(),
...
% (request, for synchronicity)
%
-spec notifyResilienceAgentsOfProbes( wooper:state(), [ pid() ] ) ->
			...
			case BasicProbeCount of
				...
				0 ->
					?info_fmt( "At simulation start, only virtual probes "
							   "were declared, there were ~B of them:~s",
...
manage_all_producers( State ) ->
	...
	ManagedState = case ?getAttr(result_spec) of
		...
		_Other ->
			...
			%io:format( "Managing result queues:~n" ),
...
browseResultReports( State ) ->
	...
	case ?getAttr(result_found) of
		...
		true ->
			...
			{ Files, _Directories, _OtherFiles, _Devices } =
			...
			case file_utils:filter_by_extensions( Files,
				...
				[] ->
					io:format( "(results matched the specification, "
							   "but none was graphical, thus nothing to "
							   "be browsed here)~n" );

				L ->
					io:format( "(displaying ~B graphical results)~n",
							   [ length( L ) ] ),
...
% (oneway)
%
-spec addResultListener( wooper:state(), pid() ) -> oneway_return().
