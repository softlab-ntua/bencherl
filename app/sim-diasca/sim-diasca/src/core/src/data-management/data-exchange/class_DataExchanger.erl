...
% TO-DO: trigger appropriately ?hashtable_type:optimise/1 calls on data_table for
% resizing due to dynamic definitions, that could be numerous (ex: use
% milestones).
...
%
-spec construct( wooper_state(), string(),
...
construct( State, ExchangerName, {ConfigurationFileList,RootTimeManagerPid} )
  when is_list(ConfigurationFileList) andalso is_pid(RootTimeManagerPid) ->
	...
	setAttributes( ReadState, [
		...
		{ feeder_files, text_utils:strings_to_binaries(ConfigurationFileList) },
...
construct( State, ExchangerName, {ParentExchangerPid,NodeType} )
  when is_pid(ParentExchangerPid) andalso is_atom(NodeType) ->
	...
	?send_debug_fmt( NewState, "Data table after construction: ~s.",
			  [ ?hashtable_type:toString( getAttribute(NewState,data_table) ) ] ),
...
delete( State ) ->
	...
	delete_synchronously_instances( ?getAttr(child_exchangers) ),
...
% (request)
%
-spec synchronise( wooper_state() ) -> request_return(
												  { [ binary() ], pid() } ).
synchronise( State ) ->
...
% (oneway)
%
-spec parse( wooper_state(), [file_utils:file_name()] ) -> oneway_return().
parse( State, FileList ) ->

	BasePath = class_Actor:get_deployed_root_directory( State ),

	AbsolutePathList = [ file_utils:join(BasePath,Path) || Path <- FileList ],
...
simulation_started( State ) ->
	...
	?wooper_return_state_only( setAttribute( State, simulation_running, true )
							  ).
...
simulation_stopped( State ) ->
	none = ?getAttr(parent_exchanger_pid),
	?wooper_return_state_only( setAttribute( State, simulation_running, false )
							  ).
...
%
-spec onInterDiascaBegin( wooper_state() ) ->
...
% (request)
%
-spec defineInitialData( wooper_state(), entries() ) ->
...
% (request)
%
-spec defineInitialData( wooper_state(), key(), value() ) ->
							   request_return( 'initial_data_defined' ).
...
% (request)
%
-spec defineInitialData( wooper_state(), key(), value(), qualifier() ) ->
							   request_return( 'initial_data_defined' ).
...
% (request)
%
-spec modifyInitialData( wooper_state(), entries() )  ->
...
% (request)
%
-spec modifyInitialData( wooper_state(), key(), value() ) ->
							request_return( 'initial_data_modified' ).
...
% (request)
%
-spec modifyInitialData( wooper_state(), key(), value(), qualifier() ) ->
							request_return( 'initial_data_modified' ).
...
% (const request)
%
-spec readInitialData( wooper_state(), [ key() ] ) ->
...
% (const request)
%
-spec readQualifiedInitialData( wooper_state(), key() ) ->
...
% (request)
%
-spec defineData( wooper_state(), entries() ) ->
...
% (request)
%
-spec defineData( wooper_state(), key(), value() ) ->
						request_return( 'data_defined' ).
...
% (request)
%
-spec defineData( wooper_state(), key(), value(), qualifier() ) ->
						request_return( 'data_defined' ).
...
% (request)
%
-spec modifyData( wooper_state(), entries() ) ->
...
% (request)
%
-spec modifyData( wooper_state(), key(), value() ) ->
						request_return( 'data_modified' ).
...
% (request)
%
-spec modifyData( wooper_state(), key(), value(), qualifier() ) ->
						request_return( 'data_modified' ).
...
% (const request)
%
-spec readData( wooper_state(), key() ) -> request_return( value() );
			  ( wooper_state(), [ key() ] ) -> request_return( entries() ).
...
% (const request)
%
-spec readQualifiedData( wooper_state(), key() ) ->
							   request_return( qualified_value() );
					   ( wooper_state(), [ key() ] ) ->
							   request_return( qualified_entries() ).
...
% (request)
%
-spec defineDataHelper( wooper_state(), entry() | entries() ) ->
					request_return( 'data_defined' ).
...
% (request)
%
-spec modifyDataHelper( wooper_state(), entry() | entries() ) ->
							request_return( 'data_modified' ).
...
% (request)
%
-spec commitDataHelper( wooper_state(), qualified_basic_entries() ) ->
...
% (const request)
%
-spec getAllData( wooper_state() ) -> request_return( entries() ).
getAllData( State ) ->
...
% (const request)
%
-spec traceData( wooper_state() ) -> request_return( 'data_traced' ).
traceData( State ) ->
	DataPairs = ?hashtable_type:enumerate( ?getAttr(data_table) ),
	PairStrings = [ io_lib:format( "~p: ~p", [K,V] ) || {K,V} <- DataPairs ],
...
% (const request)
%
-spec traceDistributedData( wooper_state() ) ->
...
% (helper function)
%
-spec manage_inter_diasca_notification( wooper_state() ) -> wooper_state().
...
% (helper function)
%
-spec common_construct( string(), wooper_state() ) -> wooper_state().
...
% (helper function)
%
-spec parse_files( [ file_utils:file_name() ], node_type(), wooper_state() ) ->
						 wooper_state().
parse_files( FileList, NodeType, State ) ->
	...
	% Optimising the load factor helps a lot the ?hashtable_type look-up performances:
...
% (request)
%
-spec executeRequestInTree( wooper_state(), method_name(), method_arguments() )
						  -> request_return( any() ).
...
-spec executeRequestInTreeHelper( wooper_state(), method_name(),
