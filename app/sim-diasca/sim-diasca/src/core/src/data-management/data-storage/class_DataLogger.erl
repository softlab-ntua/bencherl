...
%
-spec construct( wooper_state(), registration_options(),
...
% (request)
%
-spec createVirtualProbe( wooper_state(), text_utils:bin_string(),
...
% (oneway)
%
-spec setData( wooper_state(), virtual_probe_id(), probe_tick(),
...
% (request)
%
-spec setDataSynchronous( wooper_state(), virtual_probe_id(),
...
% (oneway)
%
-spec mergeData( wooper_state(), virtual_probe_id(), probe_tick(),
			  sample_data() ) -> oneway_return().
...
% (request)
%
-spec mergeDataSynchronous( wooper_state(), virtual_probe_id(),
		 probe_tick(), sample_data() ) ->
...
% (oneway)
%
-spec addCurve( wooper_state(), virtual_probe_id(),
...
% (const request)
%
-spec getCurveRenderOrder( wooper_state(), virtual_probe_id() ) ->
...
% (oneway)
%
-spec setCurveRenderOrder( wooper_state(), virtual_probe_id(), [ string() ] ) ->
								 oneway_return().
...
% (const request)
%
-spec getProbeTable( wooper_state(), virtual_probe_id() ) ->
...
% (oneway)
%
-spec setPlotStyle( wooper_state(), virtual_probe_id(), string() ) ->
						  oneway_return().
...
% (oneway)
%
-spec setFillStyle( wooper_state(), virtual_probe_id(), string() ) ->
						  oneway_return().
...
% (oneway)
%
-spec setCanvasSize( wooper_state(), virtual_probe_id(), gui:length(),
...
% (oneway)
%
-spec setKeyOptions( wooper_state(), virtual_probe_id(), string() ) ->
						   oneway_return().
...
%
-spec sendResults( wooper_state(), list() ) ->
...
sendResults( State, _Options ) ->
	...
	case Files of
		...
		[] ->
			?wooper_return_state_result( State, { self(), no_result } );
		...
		FilenameList ->
			...
			?wooper_return_state_result( State, { self(), archive, Bin } )
...
% (request)
%
-spec generateReport( wooper_state(), virtual_probe_id() ) ->
...
% (request)
%
-spec generateReport( wooper_state(), virtual_probe_id(), boolean() ) ->
			request_return( 'probe_report_generated' ).
...
% (helper function)
%
-spec ensure_database_activated( wooper_state() ) -> wooper_state().
...
% (helper function)
%
-spec get_virtual_probe( virtual_probe_id(), wooper_state() ) ->
							   virtual_probe().
...
% (helper function)
%
-spec get_virtual_probe_and_options( virtual_probe_id(), wooper_state() ) ->
...
-spec set_virtual_probe( virtual_probe_id(),
	   { virtual_probe(), probe_options() }, wooper_state() ) -> wooper_state().
...
% (helper function)
%
-spec generate_report_from_id( virtual_probe_id(), wooper_state() ) ->
...
% (helper function)
%
-spec generate_report( virtual_probe(), wooper_state() ) ->
   { file_utils:file_name(), file_utils:file_name(), file_utils:file_name() }.
generate_report( ProbeRecord, State ) ->
