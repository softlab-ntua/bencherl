...
% A probe aggregates a series of values for a series of ticks and generates an
...
construct( State, { Name, ProbeOptions }, CurveNames, Zones, Title,
	...
	DeferredState = case DeferredDataWrites of
		 ...
		 false ->
			...
			file_utils:write( DataFile, io_lib:format(
...
% (oneway)
%
-spec addCurve( wooper:state(), string() ) -> oneway_return().
...
% (const request)
%
-spec getCurveRenderOrder( wooper:state() ) -> request_return( [ string() ] ).
...
% (oneway)
%
-spec setCurveRenderOrder( wooper:state(), [ string() ] ) -> oneway_return().
...
% (oneway)
%
-spec setPlotStyle( wooper:state(), string() ) -> oneway_return().
...
% (oneway)
%
-spec setPlotStyle( wooper:state(), string(), boolean() ) -> oneway_return().
...
% (oneway)
%
-spec setFillStyle( wooper:state(), string() ) -> oneway_return().
...
% (oneway)
%
-spec setFillStyle( wooper:state(), string(), boolean() ) ->
	 oneway_return().
...
% (oneway)
%
-spec setCanvasSize( wooper:state(), gui:length(), gui:length() ) ->
						   oneway_return().
...
% (oneway)
%
-spec setCanvasSize( wooper:state(), gui:length(), gui:length(), boolean() ) ->
						   oneway_return().
...
% (oneway)
%
-spec setKeyOptions( wooper:state(), string() ) -> oneway_return().
...
% (oneway)
%
-spec setKeyOptions( wooper:state(), string(), boolean() ) -> oneway_return().
...
% (oneway)
%
-spec setAbscissaRange( wooper:state(), gui:coordinate(), gui:coordinate() ) ->
							  oneway_return().
...
% (oneway)
%
-spec setOrdinateRange( wooper:state(), gui:coordinate(), gui:coordinate() ) ->
							 oneway_return().
...
% (oneway)
%
-spec addLabel( wooper:state(), string(), gui:point() ) -> oneway_return().
...
% (oneway)
%
-spec addLabel( wooper:state(), string(), gui:point(), gui_color:color() ) ->
					  oneway_return().
...
%
-spec addLabel( wooper:state(), string(), gui:point(), gui_color:color(),
			label_orientation(), label_position() ) -> wooper:state().
...
% (oneway)
%
-spec setDirectory( wooper:state(), file_utils:directory_name() ) ->
						  oneway_return().
...
% Sends the specified type of results to the caller (generally the result
% manager).
%
...
sendResults( State, [ data_only ] ) ->
	...
	% We consider these results are produced, as only data is required and is
	% already available:
	%
	FinalState = setAttributes( CommandState, [

			{ result_produced, true },
			{ result_collected, true } ] ),

	?wooper_return_state_result( FinalState,
...
sendResults( State, [ plot_only ] ) ->
	...
	?wooper_return_state_result( FinalState, { self(), raw,
...
sendResults( State, [ data_and_plot ] ) ->
	...
	?wooper_return_state_result( FinalState, { self(), archive, BinArchive } ).
...
%
%  - or { Name :: string(), ProbeOptions } where ProbeOptions is a list of
%  pairs, in:
%
%   - { create_command_file_initially, boolean() }: if true, the gnuplot command
%   file will be written at probe start-up, thus preventing the taking into
...
%   false
%
% - CurveNames :: [ string() ] is an (ordered) list containing the names (as
% plain strings) of each curve to be drawn (hence the probe will expect
% receiving data in the form of { Tick, {V1,V2,..} }). For example,
% CurveNames=["First curve","Second curve"] will lead to expect to receive
...
% be created (unilaterally) from a test case, and that will not to considered as
% a result.
%
% - NameOptions is either:
%
...
delete_facility_probe( ProbePid ) ->
	...
	% This is necessarily a PID, not a 'non_wanted_probe' atom; synchronousl is
...
generate_data_file( State ) ->
	...
	file_utils:write( File, FormattedData ),
...
% (oneway)
%
-spec setFilledCurvesOptions( wooper:state(), string(), curve_index() ) ->
									wooper:state().
...
generate_report( Name, State ) ->
	...
	case ?getAttr(result_produced) of
		...
		true ->
			?warning_fmt( "Report '~s' has already been generated "
						  "at least once.", [ Name ] );
