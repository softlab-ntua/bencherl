...
%
-spec construct( wooper_state(), basic_utils:registration_name(),
...
delete( State ) ->
	...
	send_to_all_probes( delete, TickerState ),
...
% (oneway)
%
-spec start( wooper_state(), pid(), [ net_utils:atom_node_name() ],
...
% (oneway)
%
-spec stop( wooper_state() ) -> oneway_return().
stop( State ) ->
...
% (helper)
%
-spec stop_ticker( wooper_state() ) -> wooper_state().
stop_ticker( State ) ->
...
% (oneway)
%
-spec setTickerPeriod( wooper_state(), unit_utils:milliseconds() ) ->
							 oneway_return().
...
% (request)
%
-spec generateMonitoringReports( wooper_state() ) ->
...
%
-spec record_new_sample( wooper_state() ) -> basic_utils:void().
...
reorder_classes( InstancesPerClass, _OrderedClasses=[Class|T], ExistingAcc ) ->
...
%
-spec wait_dynamic_info_from_trackers( [ pid() ], wooper_state(),
...
% (helper)
%
-spec send_to_all_probes( any(), wooper_state() ) -> [ pid() ].
send_to_all_probes( Message, State ) ->

	SingleProbeList = [ ?getAttr(nodes_in_tick_probe),
						?getAttr(nodes_in_time_probe),
						?getAttr(classes_in_tick_probe),
						?getAttr(classes_in_time_probe) ],
...
pre_serialise_hook( State ) ->
	...
	% markers:
	%
	handle_private_processes( PrivateProcesses, State ).
