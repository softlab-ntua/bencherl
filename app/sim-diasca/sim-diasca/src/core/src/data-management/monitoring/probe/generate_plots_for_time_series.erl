...
-spec run( file_utils:directory_name() ) -> basic_utils:void().
run( Dir ) ->
...
select_data_files( DirectoryName ) ->
	...
	lists:sort( filter_by_extension( RegularFiles, ".dat" ) ).
	...
	case is_existing_file( CommandFilename ) of
		...
		true ->
			...
			case generate_report( DataFilename, CommandFilename ) of
				...
				{failure,Reason} ->
					DispatcherPid ! {work_failed,
									 self(), {DataFilename,Reason} }
...
generate_report( DataFilename, CommandFilename ) ->
	...
	io:format( " - generating report for ~s~n", [DataFilename] ),
...
string_list_to_string( _ListOfStrings=[H|T], Acc, Bullet ) when is_list(H) ->
