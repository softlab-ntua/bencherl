...
% (oneway)
%
-spec addStaticLink( wooper:state(), pure_node(), pure_node(), link_content() )
				   -> oneway_return().
...
% (const request)
%
-spec getAllLinks( wooper:state() ) -> request_return( [ any_link() ] ).
getAllLinks( State ) ->
...
% (oneway)
%
-spec pruneFrom( wooper:state(), pure_node() ) -> oneway_return().
...
% (oneway)
%
-spec pruneTo( wooper:state(), pure_node() ) -> oneway_return().
pruneTo( State, Node ) ->
...
% (oneway)
%
-spec addRenderingRawElement( wooper:state(), string() ) -> oneway_return().
...
% (const request)
%
-spec getMeshInformation( wooper:state() ) -> request_return( string() ).
...
% (request)
%
-spec generateTopologicalView( wooper:state(), boolean(), string() ) ->
				  request_return( 'topological_view_generated' ).
...
generate_topological_view( DisplayWanted, BaseFileName, State ) ->
	...
	setAttribute( DirState, result_produced, true ).
