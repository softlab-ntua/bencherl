...
% (const request)
%
-spec getNodeName( wooper:state() ) -> request_return( string() ).
getNodeName( State ) ->
...
% (const request)
%
-spec getLabel( wooper:state() ) -> request_return( string() ).
getLabel( State ) ->
...
% (oneway)
%
-spec setLabel( wooper:state(), string() ) -> oneway_return().
...
% (const request)
%
-spec getGraphOptions( wooper:state() ) -> request_return( options() ).
getGraphOptions( State ) ->
	?wooper_return_state_result( State,	select_attributes_from( State ) ).
...
% (const actor oneway)
%
-spec getGraphOptions( wooper:state(), pid() ) ->
