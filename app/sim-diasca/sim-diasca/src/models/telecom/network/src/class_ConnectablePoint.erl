...
-spec construct( wooper_state(), class_Actor:actor_settings(),
				class_Actor:name(), pid() | 'undefined' ) -> wooper_state().
...
% (const request)
%
-spec getConnectedDevice( wooper_state() ) ->
...
% (oneway)
%
-spec connectDevice( wooper_state(), pid() ) -> oneway_return().
...
% (const request)
%
-spec getGraphInformations( wooper_state() ) ->
