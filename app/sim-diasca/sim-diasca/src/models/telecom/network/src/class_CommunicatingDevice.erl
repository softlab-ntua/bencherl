...
%
-spec construct( wooper_state(), string(), pid() ) -> wooper_state().
construct( State, ?wooper_construct_parameters ) ->

	% First the direct mother classes, then this class-specific actions:
	% (no option list of abstract devices)
	GraphState = class_Graphable:construct( State, [ {label,DeviceName} ] ),
...
% (oneway)
%
-spec setNetworkListener( wooper_state(), pid() ) -> oneway_return().
