==========================
Sim-Diasca Developer Guide
==========================



+------------------------------------------------------------+------------------------------+
| .. image:: sim-diasca.png                                                                 |
|   :scale: 40                                                                              |
|   :align: center                                                                          |
+============================================================+==============================+
| .. image:: logo-EDF-english.png                            | .. image:: lgplv3-147x51.png |
|   :scale: 50                                               |   :scale: 20                 |
|   :align: left                                             |   :align: right              |
+------------------------------------------------------------+------------------------------+


.. role:: raw-html(raw)
   :format: html

.. role:: raw-latex(raw)
   :format: latex


:Author: Olivier Boudeville
:Contact: olivier.boudeville@edf.fr
:Organisation: Copyright (C) 2008-2014 EDF R&D
:Creation Date: February 2009
:Lastly Updated on: Wednesday, October 1, 2014
:Status: Work in progress
:Version: 0.9
:Dedication: To the Sim-Diasca developers


:Abstract:

	This document summarizes the main conventions that should be respected when contributing code to Sim-Diasca and/or making use of Sim-Diasca.



:raw-latex:`\pagebreak`

.. contents:: Table of Contents
	:depth: 4

.. section-numbering::


:raw-latex:`\pagebreak`

.. Note::

 This document intends to gather information aimed at Sim-Diasca *maintainers*, not users. Most people should read the *Sim-Diasca Technical Manual* first.

 However we require internally that the authors of any simulation element making use of Sim-Diasca's services (ex: models, simulation cases, etc.) respect these conventions as well, for clarity and homogeneity.

.. include:: SimDiasca-code-conventions-english.rst


:raw-latex:`\pagebreak`

.. include:: SimDiasca-SVN-conventions-english.rst

.. include:: SimDiasca-SVN-guide-english.rst



:raw-latex:`\pagebreak`

.. include:: SimDiasca-developer-hints-english.rst

.. include:: SimDiasca-about-type-specifications-english.rst

.. include:: SimDiasca-guide-footer-english.rst
