...
%%%-------------------------------------------------------------------
%% @version $Id: nodelist_SUITE.erl 2618 2011-12-29 14:35:15Z kruber@zib.de $
-module(nodelist_SUITE).

-author('kruber@zib.de').
-vsn('$Id: nodelist_SUITE.erl 2618 2011-12-29 14:35:15Z kruber@zib.de $').
...
-define(compare_neighborhood(Neighborhood, Node, NodeId, Pred, Preds, Succ, Succs, RealPred, RealSucc),
        compare_neighborhood(??Neighborhood, Neighborhood, Node, NodeId, Pred, Preds, Succ, Succs, RealPred, RealSucc)).

-spec compare_neighborhood(NeighborhoodStr::string(), N::nodelist:neighborhood(), Node::node:node_type(), NodeId::?RT:key(), Pred::node:node_type(), Preds::nodelist:snodelist(), Succ::node:node_type(), Succs::nodelist:snodelist(), RealPred::boolean(), RealSucc::boolean()) -> ok.
compare_neighborhood(NeighborhoodStr, N, Node, NodeId, Pred, Preds, Succ, Succs, RealPred, RealSucc) ->
    ?equals_w_note(nodelist:node(N), Node, io_lib:format("N = ~s", [NeighborhoodStr])),
    ?equals_w_note(nodelist:nodeid(N), NodeId, io_lib:format("N = ~s", [NeighborhoodStr])),
...
test_remove3(_Config) ->
    ...
    ?equals(_N510 = lists:sort(ets:tab2list(RemovedNodesTab)), NodeListToTabListFun([])),
    ets:delete_all_objects(RemovedNodesTab),
    ?equals(nodelist:remove(N9, Neighb5, AddNodeToTabFun), Neighb5),
    ?equals(_N511 = lists:sort(ets:tab2list(RemovedNodesTab)), NodeListToTabListFun([])),
    ets:delete_all_objects(RemovedNodesTab),
    ?equals(nodelist:remove(pid3, Neighb5, AddNodeToTabFun), Neighb5),
    ?equals(_N512 = lists:sort(ets:tab2list(RemovedNodesTab)), NodeListToTabListFun([])),
    ets:delete_all_objects(RemovedNodesTab),
    ?equals(nodelist:remove(pid1, Neighb5, AddNodeToTabFun), Neighb5),
    ?equals(_N513 = lists:sort(ets:tab2list(RemovedNodesTab)), NodeListToTabListFun([])),
    ets:delete_all_objects(RemovedNodesTab),

    % Neighb6 = nodelist:mk_neighborhood([], N33, 1, 1),
    % remove non-existing nodes and pids:
    ?equals(nodelist:remove(N20, Neighb6, AddNodeToTabFun), Neighb6),
    ?equals(_N610 = lists:sort(ets:tab2list(RemovedNodesTab)), NodeListToTabListFun([])),
    ets:delete_all_objects(RemovedNodesTab),
    ?equals(nodelist:remove(N9, Neighb6, AddNodeToTabFun), Neighb6),
    ?equals(_N611 = lists:sort(ets:tab2list(RemovedNodesTab)), NodeListToTabListFun([])),
    ets:delete_all_objects(RemovedNodesTab),
    ?equals(nodelist:remove(pid3, Neighb6, AddNodeToTabFun), Neighb6),
    ?equals(_N612 = lists:sort(ets:tab2list(RemovedNodesTab)), NodeListToTabListFun([])),
    ets:delete_all_objects(RemovedNodesTab),
    ?equals(nodelist:remove(pid1, Neighb6, AddNodeToTabFun), Neighb6),
    ?equals(_N613 = lists:sort(ets:tab2list(RemovedNodesTab)), NodeListToTabListFun([])),
    ets:delete_all_objects(RemovedNodesTab),
...
test_filter_min_length4(_Config) ->
    ...
    ?compare_neighborhood(nodelist:filter_min_length(Neighb1, fun(N) -> not (node:same_process(N, N8) orelse node:same_process(N, N50)) end, 0, 0),
                          N33, 33, N14, [N14, N42], N42, [N42, N14], true, true),
    ...
    ?compare_neighborhood(nodelist:filter_min_length(Neighb1, fun(N) -> not (node:same_process(N, N8) orelse node:same_process(N, N50)) end, 3, 3),
                          N33, 33, N14, [N14, N8, N42], N42, [N42, N50, N14], true, true),
    ...
    ?compare_neighborhood(nodelist:filter_min_length(Neighb1, fun(N) -> not (node:same_process(N, N8) orelse node:same_process(N, N50)) end, 2, 4),
                          N33, 33, N14, [N14, N42], N42, [N42, N50, N8, N14], true, true),
    ?compare_neighborhood(nodelist:filter_min_length(Neighb1, fun(N) -> not (node:same_process(N, N8) orelse node:same_process(N, N50)) end, 4, 2),
                          N33, 33, N14, [N14, N8, N50, N42], N42, [N42, N14], true, true),
    ...
    ?compare_neighborhood(nodelist:filter_min_length(Neighb2, fun(N) -> not (node:same_process(N, N8) orelse node:same_process(N, N50)) end, 2, 4),
                          N33, 33, N14, [N14, N8], N42, [N42, N50, N8], true, true),
    ?compare_neighborhood(nodelist:filter_min_length(Neighb2, fun(N) -> not (node:same_process(N, N8) orelse node:same_process(N, N50)) end, 4, 2),
                          N33, 33, N14, [N14, N8, N50], N42, [N42, N50], true, true),
...
test_add_node(_Config) ->
    ...
    ?compare_neighborhood(nodelist:add_node(Neighb1, N20, 4, 4),
                          N33, 33, N20, [N20, N14, N8, N50], N50, [N50, N8, N14, N20], true, true),
    ?compare_neighborhood(nodelist:add_node(Neighb1, N42, 4, 4),
                          N33, 33, N14, [N14, N8, N50, N42], N42, [N42, N50, N8, N14], true, true),
    ?compare_neighborhood(nodelist:add_node(Neighb1, N66, 4, 4),
                          N33, 33, N14, [N14, N8, N66, N50], N50, [N50, N66, N8, N14], true, true),
    ...
    ?compare_neighborhood(nodelist:add_node(Neighb5, N66, 3, 3),
                          N33, 33, N14, [N14, N66], N66, [N66, N14], true, true),
    ?compare_neighborhood(nodelist:add_node(Neighb5, N9, 3, 3),
                          N33, 33, N14, [N14, N9], N9, [N9, N14], true, true),
...
%%     Neighb6 = nodelist:mk_neighborhood([], N33, 1, 1),
    ...
    ?compare_neighborhood(nodelist:add_node(Neighb6, N66, 3, 3),
                          N33, 33, N66, [N66], N66, [N66], true, true),
    ?compare_neighborhood(nodelist:add_node(Neighb6, N9, 3, 3),
                          N33, 33, N9, [N9], N9, [N9], true, true),
...
test_lremove_outdated(_Config) ->
    ...
    ?equals(nodelist:lremove_outdated([N8, Null]), [N8]),
    ?equals(nodelist:lremove_outdated([Null, N8]), [N8]),
    ...
    ?equals(nodelist:lremove_outdated([N8, N14, N20, Null, N33, N50]), [N8, N14, N20, N33, N50]),
    ?equals(nodelist:lremove_outdated([N8, N9]), [N9]),
    ?equals(nodelist:lremove_outdated([Null, N8, N9]), [N9]),
    ?equals(nodelist:lremove_outdated([N8, Null, N9]), [N9]),
    ...
    ?equals(nodelist:lremove_outdated([Null], N50), []),
    ?equals(nodelist:lremove_outdated([N8], N50), [N8]),
    ?equals(nodelist:lremove_outdated([N8], N9), []),
    ?equals(nodelist:lremove_outdated([N8, Null], N50), [N8]),
    ...
    ?equals(nodelist:lremove_outdated([N8, N14, N20, N33, N50, Null], N50), [N8, N14, N20, N33, N50]),
    ?equals(nodelist:lremove_outdated([N8, N14, N20, N33, N50, Null], N21), [N8, N14, N33, N50]),
    ?equals(nodelist:lremove_outdated([N8, N14, N20, N21, N33, N50, Null], N21), [N8, N14, N21, N33, N50]),
    ?equals(nodelist:lremove_outdated([N8, N14, N20, Null, N33, N50], N50), [N8, N14, N20, N33, N50]),
    ?equals(nodelist:lremove_outdated([N8, N9], N50), [N9]),
    ?equals(nodelist:lremove_outdated([Null, N8, N9], N50), [N9]),
    ?equals(nodelist:lremove_outdated([N8, Null, N9], N50), [N9]),
    ...
    ?equals(nodelist:lremove_outdated([N8, N20, N33, N14, Null, N50]), [N8, N20, N33, N14, N50]),
    ?equals(nodelist:lremove_outdated([N9, N8]), [N9]),
    ?equals(nodelist:lremove_outdated([Null, N9, N8]), [N9]),
    ?equals(nodelist:lremove_outdated([N9, Null, N8]), [N9]),
    ...
    ?equals(nodelist:lremove_outdated([N8, N20, N33, N14, N50, Null], N50), [N8, N20, N33, N14, N50]),
    ?equals(nodelist:lremove_outdated([N8, N20, N33, N14, Null, N50], N50), [N8, N20, N33, N14, N50]),
    ?equals(nodelist:lremove_outdated([N8, N20, N33, N14, Null, N50], N21), [N8, N33, N14, N50]),
    ?equals(nodelist:lremove_outdated([N8, N20, N33, N14, Null, N21, N50], N21), [N8, N33, N14, N21, N50]),
    ?equals(nodelist:lremove_outdated([N9, N8], N50), [N9]),
    ?equals(nodelist:lremove_outdated([Null, N9, N8], N50), [N9]),
    ?equals(nodelist:lremove_outdated([N9, Null, N8], N50), [N9]),
