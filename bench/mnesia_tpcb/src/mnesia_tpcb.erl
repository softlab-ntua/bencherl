...
bench_args() ->
	[[Drivers,Accounts,Branches] || Drivers <- [20], Accounts <- [40], Branches <- [2000]].
...
run([Drivers,Accounts,Branches|_], _, _) ->
	Nodes = [node()],
	S = 20,
    Config = [	{db_nodes, Nodes},
				{driver_nodes, Nodes}, 
				{replica_nodes, Nodes}, 
				{n_drivers_per_node, Drivers}, 
				{n_branches, Branches}, 
				{n_accounts_per_branch, Accounts}, 
				{replica_type, ram_copies}, 
				{stop_after, timer:seconds(S)}, 
				%% {report_interval, timer:seconds(10)}, 
				{reuse_history_id, true}],
	mnesia_tpcb:init([{use_running_mnesia, false}|Config]),
    {ok, #time{ n_trans = Trans }} = mnesia_tpcb:run([{use_running_mnesia, true}|Config]),
	mnesia_tpcb:stop(),
	Tps = Trans/S,
	io:format("~p tps~n", [Tps]),
	ok.
...
list2rec(List, [F|Fields], [D|Defaults], Acc) ->
	...
	case lists:keysearch(F, 1, List) of
		false -> {D, List};
		{value, {F, NewVal}} -> {NewVal, lists:keydelete(F, 1, List)}
	end,
...
sync_stop(Pid) ->
	...
	receive
		{Pid, {stopped, Res}} -> Res;
		Else -> exit({stop_error, Else})
		after timer:minutes(1) -> exit(Pid, kill), {error, brutal_kill}
	end.
...
init(Args) ->
	...
	TabConfig = if 
			TabConfig0#tab_config.n_fragments =:= 0 ->TabConfig0#tab_config{n_replicas = length(TabConfig0#tab_config.replica_nodes)};
	    	true -> TabConfig0	
		end,
	Tags = record_info(fields, tab_config),
	Fun = fun(F, Pos) -> {{F, element(Pos, TabConfig)}, Pos + 1} end,
	{_List, _} = lists:mapfoldl(Fun, 2, Tags),
	%% io:format("TPC-B: Table config: ~p ~n", [List]),

	DbNodes = TabConfig#tab_config.db_nodes,
	stop(),
	if 
		TabConfig#tab_config.use_running_mnesia =:= true -> ignore;
		true -> rpc:multicall(DbNodes, mnesia, lkill, []),
				case mnesia:delete_schema(DbNodes) of
					ok -> case mnesia:create_schema(DbNodes) of
						ok -> {Replies, BadNodes} = rpc:multicall(DbNodes, mnesia, start, []),
							case [Res || Res <- Replies, Res =/= ok] of
								[] when BadNodes =:= [] -> ok;
								BadRes -> 	io:format("TPC-B: <ERROR> Failed to start ~p: ~p~n", [BadNodes, BadRes]),
											exit({start_failed, BadRes, BadNodes})
							end;
						{error, Reason} ->	io:format("TPC-B: <ERROR> Failed to create schema on disc: ~p~n", [Reason]),
											exit({create_schema_failed, Reason})
					end;
					{error, Reason} -> 	io:format("TPC-B: <ERROR> Failed to delete schema on disc: ~p~n", [Reason]),
...
gen_tabs(TC) ->
	create_tab(TC, branch, record_info(fields, branch), undefined),
	create_tab(TC, account, record_info(fields, account), {branch, #account.branch_id}),
	create_tab(TC, teller, record_info(fields, teller), {branch, #teller.branch_id}),
	create_tab(TC, history, record_info(fields, history), {branch, #history.branch_id}),

	_NB = TC#tab_config.n_branches,
	_NT = TC#tab_config.n_tellers_per_branch,
	_NA = TC#tab_config.n_accounts_per_branch,
...
%	      [size(term_to_binary(default_history(TC)))]),
	...
	case verify_tabs() of
		ok -> ignore;
		{error, Reason} -> 	io:format("TPC-B: <ERROR> Inconsistent tables: ~w~n", [Reason]),	
...
create_tab(Name, Def) ->
	...
	case mnesia:create_table(Name, Def) of
		{atomic, ok} -> ok;
		{aborted, Reason} -> 	io:format("TPC-B: <ERROR> failed to create table ~w ~w: ~p~n", [Name, Def, Reason]),
...
run(Args) ->
	...
	receive
		{Pid, {stopped, Res}} -> Res; % Stopped by other process
		Else -> {tpcb_got, Else}   
		after RunConfig#run_config.stop_after -> sync_stop(Pid)
	end.
...
reporter_init(Starter, RC) ->
	...
	if
		...
		true -> {Replies, BadNodes} = rpc:multicall(DbNodes, mnesia, start, []),
				case [Res || Res <- Replies, Res =/= ok] of
					[] when BadNodes =:= [] -> ok;
					BadRes -> 	io:format("TPC-B: <ERROR> Failed to start ~w: ~p~n", [BadNodes, BadRes]),
								exit({start_failed, BadRes, BadNodes})
				end,
				verify_tabs()
	end,
	...
	case catch reporter_loop(State) of
		{'EXIT', Reason} -> io:format("TPC-B: Abnormal termination: ~p~n", [Reason]),
							if
								RC#run_config.use_running_mnesia =:= true -> ignore;
								true -> rpc:multicall(DbNodes, mnesia, lkill, [])
							end,
							...
							exit(shutdown);
		{ok, Stopper, State2} -> 	Time = State2#reporter_state.acc,
									Res = case verify_tabs() of
										ok -> {ok, Time};
										{error, Reason} -> 	io:format("TPC-B: <ERROR> Inconsistent tables, ~p~n", [{error, Reason}]),
															{error, Reason}
									end,
									if	
										RC#run_config.use_running_mnesia =:= true -> ignore;
									...
									if
										Stopper =/= Starter -> Stopper ! {self(), {stopped, Res}};
										true -> ignore
									end,
									exit(shutdown)
	end.
...
table_storage(Tab) ->
	case mnesia:table_info(branch, frag_properties) of
		...
		Props -> 	{value, NFO} = lists:keysearch(n_fragments, 1, Props),	
...
reporter_loop(State) ->
	...
	receive
		{From, stop} -> {ok, From, call_drivers(State, stop)};
		{'EXIT', Pid, Reason} when Pid =:= State#reporter_state.starter_pid -> 	%% call_drivers(State, stop),
																				exit({starter_died, Pid, Reason})
		after RC#run_config.report_interval -> 	Iters = State#reporter_state.n_iters,
												State2 = State#reporter_state{n_iters = Iters + 1},	
												case call_drivers(State2, report) of
													State3 when State3#reporter_state.driver_pids =/= [] -> State4 = State3#reporter_state{curr = #time{}},
...
call_drivers(State, Msg) ->
	...
	case Msg =:= stop of
		true -> Acc = State2#reporter_state.acc,
				Init = State2#reporter_state.init_micros,
				show_report(State2#reporter_state{n_iters = 0, curr = Acc, prev_micros = Init});
		false -> ignore
	end,
	State2.
...
calc_reports([Pid|Drivers], State) ->
	receive
		{'EXIT', P, Reason} when P =:= State#reporter_state.starter_pid -> exit({starter_died, P, Reason});
		{'EXIT', Pid, Reason} -> exit({driver_died, Pid, Reason});
		{Pid, Time} when is_record(Time, time) -> 	%% io:format("~w: ~w~n", [Pid, Time]),
...
show_report(State) ->
	...
	{_DiffSign, _DiffTps} = signed_diff(Iters, Tps, PrevTps),
	_Unfairness = Time#time.max_n / Time#time.min_n,
	BruttoAvg = (Now - State#reporter_state.prev_micros) div N,
	_BruttoTps = ?SECOND div BruttoAvg,
%    case Iters > 0 of
%	true ->
...
start_drivers(RC, TC) ->
	...
	Specs = [DS#driver_state{driver_id = Id, driver_node = N} || N <- Nodes, Id <- lists:seq(1, ND)],
	Specs2 = lists:sort(lists:flatten(Specs)),
	{Specs3, _OrphanBranches} = alloc_local_branches(AllBranches, Specs2, []),
...
driver_init(DS, AllBranches) ->
	...
	DS2 =
	if
		DS#driver_state.n_local_branches =:= 0 ->	DS#driver_state{	seed = Seed, 
																		n_local_branches = length(AllBranches),
																		local_branches   = AllBranches	};
		true -> DS#driver_state{seed = Seed}	
	end,
...
calc_trans(DS) ->
	...
	case DS#driver_state.reuse_history_id of
		false -> 	HistoryId = DS#driver_state.history_id + 1,
					DS2#driver_state{time=Time2, history_id = HistoryId};
		true ->		DS2#driver_state{time=Time2}
	end.
...
time_trans(DS) ->
	...
	case Res of
		AccountBal when is_integer(AccountBal) -> {Time, DS#driver_state{seed = NewSeed}};
		Other -> exit({crash, Other, Args, Random, DS})
	end.
...
random_to_args(Random, DS) ->
	...
	BranchId = case TC#tab_config.n_fragments of
			0 -> BranchPos - 1;
	...
	if
		...
		Random >= 0.85, NB > 1 ->	TmpAccountId= trunc(Random * (NB - 1) * NA),
									TmpAccountBranchId = TmpAccountId div NA,
									if
										TmpAccountBranchId =:= BranchId -> {TmpAccountBranchId + 1, TmpAccountId + NA};
										true -> {TmpAccountBranchId, TmpAccountId}
									end;
        %% Pick from a local account
		true ->	RelativeAccountId = trunc(Random * NA),
...
real_trans(RC, BranchId, Fun, Args, Mod) ->
	...
	case RC#run_config.spawn_near_branch of	
		false -> mnesia:activity(Type, Fun, Args, Mod);
		true ->	Node = table_info({branch, BranchId}, where_to_read),
...
trans_type(TC, RC) ->
	if
		TC#tab_config.n_fragments =:= 0, RC#run_config.use_sticky_locks =:= false -> {fun add_delta/7, mnesia};
		TC#tab_config.n_fragments =:= 0, RC#run_config.use_sticky_locks =:= true -> {fun sticky_add_delta/7, mnesia};
		TC#tab_config.n_fragments > 0, RC#run_config.use_sticky_locks =:= false -> {fun frag_add_delta/7, mnesia_frag}
	end.
...
verify_tabs() ->
	...
	case lists:member(node(), Nodes) of
		true ->	Tabs = [branch, teller, account, history],
				%io:format("TPC-B: Verifying tables: ~w~n", [Tabs]),
				rpc:multicall(Nodes, mnesia, wait_for_tables, [Tabs, infinity]), Fun = fun() ->	
					...
					{Res, BadNodes} = rpc:multicall(Nodes, ?MODULE, count_balance, []),
		...
		false -> {error, "Must be initiated from a running db_node"}
	end.
...
check_balance(Summaries, []) ->
	...
	case [S ||  S <- Rest, S#summary.balance =/= Balance] of
		[] -> ok;
		BadSummaries -> mnesia:abort({"Bad balance", One, BadSummaries})
	end;
